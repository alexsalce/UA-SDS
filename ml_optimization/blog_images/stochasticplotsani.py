...
line_sgd, = ax_path.plot([], [], 'y-', label='SGD Path')
line_adam, = ax_path.plot([], [], 'm-.', label='ADAM Path')
...
def init_path():
    ...
    return line_sarah, line_svrg, line_sgd, line_adam, true_point

def update_path(frame):
    sarah_data = sarah_solution_path[:frame+1]
    svrg_data = svrg_solution_path[:frame+1]
    sgd_data = sgd_solution_path[:frame+1]
    adam_data = adam_solution_path[:frame+1]
    
    line_sarah.set_data(sarah_data[:, 0], sarah_data[:, 1])
    line_svrg.set_data(svrg_data[:, 0], svrg_data[:, 1])
    line_sgd.set_data(sgd_data[:, 0], sgd_data[:, 1])
    line_adam.set_data(adam_data[:, 0], adam_data[:, 1])
...
# ani_path.save('stochasticgds.gif', writer='ffmpeg', fps=100)
ani_path.save('stochasticgds.gif', writer='MovieWriter', fps=100)
